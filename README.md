...
# Добрый день! 
## Сдаю Домашнее задание по теме "Знакомство с API на примере Django REST framework"
## Отрабатывают все запросы из requests.http. Но POST запрос, добавляющий измерения, отрабатывает с ошибкой:
### AttributeError: 'NoneType' object has no attribute '_meta'
## при этом программа не падает, а измерения успешно записываются в БД. 
## Гуглил, на сайте stackoverflow подымался подобный вопрос, сказано что возникает какая-то путаница с моделью,
## но я так и не смог до конца разобраться. Подскажите пожалуйста.
